...
void ApplyImGuiStyle() {
	...
	colors[ImGuiCol_TextSelectedBg] =
			ImVec4(0.2f, 0.15f, 0.5, 0.92f); // ImGui::ColorConvertU32ToFloat4((BLUE400 & 0x00FFFFFF) | 0x33000000);
	colors[ImGuiCol_DragDropTarget] = ImVec4(1.00f, 1.00f, 0.00f, 0.90f);
	colors[ImGuiCol_NavHighlight] =
			ImVec4(0.69f, 1, 1, 1); // ImGui::ColorConvertU32ToFloat4((GRAY900 & 0x00FFFFFF) | 0x0A000000);
}

void Gui::LoadFonts(const float fontSizeMultiplier) {
	const ImGuiIO &io = ImGui::GetIO();

	ImFontConfig config;
	config.OversampleH = static_cast<ImS8>(2 * fontSizeMultiplier);
	config.OversampleV = static_cast<ImS8>(2 * fontSizeMultiplier);
	...
	DEBUG_PRINT("Loaded Courier Prime\n");
	...
	io.Fonts->AddFontFromFileTTF(R"(C:\Windows\Fonts\Framd.ttf)", defaultFontSize, &config);

	io.Fonts->AddFontFromFileTTF(R"(C:\Windows\Fonts\Lucon.ttf)", defaultFontSize, &config);
...
HWND Gui::Setup(int (*OnGuiFunc)()) {
	...
	wc = {sizeof(WNDCLASSEX),
		  ...
		  nullptr,
		  nullptr,
		  _T("Latency Meter Refreshed"),
		  nullptr};
	wc.hIcon = LoadIcon(wc.hInstance, MAKEINTRESOURCE(IDI_ICON1));
	wc.hIconSm = LoadIcon(wc.hInstance, MAKEINTRESOURCE(IDI_ICON1));
	::RegisterClassEx(&wc);
	hwnd = ::CreateWindow(wc.lpszClassName, _T("Latency Meter Refreshed"),
						  (WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_THICKFRAME), 100, 100,
						  WINDOW_X, WINDOW_Y, nullptr, nullptr, wc.hInstance, nullptr);
	...
	if (g_pd3dDevice->QueryInterface(__uuidof(IDXGIDevice), reinterpret_cast<void **>(&pDXGIDevice)) != S_OK) {
		CleanupDeviceD3D();
		::UnregisterClass(wc.lpszClassName, wc.hInstance);
	...
	if (pDXGIAdapter->GetParent(__uuidof(IDXGIFactory), reinterpret_cast<void **>(&pIDXGIFactory)) != S_OK) {
		CleanupDeviceD3D();
		::UnregisterClass(wc.lpszClassName, wc.hInstance);
		std::cerr << "Failed to get IDXGIFactory" << std::endl;
		return nullptr;
	}

	if (pIDXGIFactory->MakeWindowAssociation(hwnd, DXGI_MWA_NO_WINDOW_CHANGES | DXGI_MWA_NO_ALT_ENTER) != S_OK) {
		CleanupDeviceD3D();
		::UnregisterClass(wc.lpszClassName, wc.hInstance);
		std::cerr << "Failed to make window association, expect issues with fullscreen" << std::endl;
	}
...
int Gui::DrawGui() noexcept {
	...
	ImGui::Begin("Window", &showMainWindow,
	...
	if (const int mainGui = mainGuiFunc())
		return mainGui;
	...
	const auto presentRes = g_pSwapChain->Present(*g_vSyncFrame, 0);
...
bool Gui::GetFullScreenState() {
	BOOL state = false;
	g_pSwapChain->GetFullscreenState(&state, nullptr);

	return state;
}

int Gui::SetFullScreenState(const BOOL state) { return g_pSwapChain->SetFullscreenState(state, nullptr); }
...
bool CreateDeviceD3D(const HWND hWnd) {
	...
	while (pIDXGIFactory->EnumAdapters(iAdapter, &pAdapter) != DXGI_ERROR_NOT_FOUND) {
		...
		while (pAdapter->EnumOutputs(iOutput, &pOutput) != DXGI_ERROR_NOT_FOUND) {
			...
			if (pOutput->GetDisplayModeList(DXGI_FORMAT_R10G10B10_XR_BIAS_A2_UNORM, 0, &modesNum, monitorModes) != S_OK)
				modesNum = 0;

#ifdef _DEBUG
			for (UINT i = 0; i < modesNum; i++) {
				printf("Mode %i: %ix%i@%iHz\n", i, monitorModes[i].Width, monitorModes[i].Height,
					   (monitorModes[modesNum - 1].RefreshRate.Numerator /
						monitorModes[modesNum - 1].RefreshRate.Denominator));
			}
#endif
...
LRESULT WINAPI WndProc(const HWND hWnd, const UINT msg, const WPARAM wParam, const LPARAM lParam) {
	...
	switch (msg) {
		case WM_SETFOCUS:
			if (KeyDownFunc)
				KeyDownFunc(static_cast<WPARAM>(-1), static_cast<LPARAM>(-1), false);
			break;
		case WM_SIZE:
		...
		case WM_GETMINMAXINFO: {
			...
			lpMMI->ptMinTrackSize.x = WINDOW_X;
			lpMMI->ptMinTrackSize.y = WINDOW_Y;
			break;
		}
		case WM_SYSCOMMAND:
			if (wParam == SC_CLOSE) {
...
int Gui::Setup(int (*OnGui)()) {
	...
	g_window = glfwCreateWindow(WINDOW_X, WINDOW_Y, "Latency Meter Refreshed", nullptr, nullptr);
	if (g_window == nullptr)
		return 1;
	glfwMakeContextCurrent(g_window);
	glfwSwapInterval(*g_vSyncFrame); // Enable vsync
...
int Gui::DrawGui() noexcept {
	if (glfwWindowShouldClose(g_window) && (!g_onExitFunc || g_onExitFunc()))
		return 1;

	glfwSwapInterval(*g_vSyncFrame);
...
int Gui::SetFullScreenState(bool state) {
	...
	if (state) {
		if (!GetFullScreenState()) {
			glfwGetWindowPos(g_window, pos, pos + 1);
			glfwGetWindowSize(g_window, size, size + 1);

			DEBUG_PRINT("Saved size = [%i, %i]\n", size[0], size[1]);
		}
		glfwSetWindowMonitor(g_window, glfwGetPrimaryMonitor(), 0, 0, mode->width, mode->height, mode->refreshRate);
	} else {
		glfwSetWindowMonitor(g_window, NULL, pos[0], pos[1], size[0], size[1], mode->refreshRate);
		glfwSetWindowSize(g_window, size[0], size[1] + MARGIN_Y);
...
bool Gui::GetFullScreenState() { return glfwGetWindowMonitor(g_window) != nullptr; }
...
bool Gui::RegKeyCallback(GLFWkeyfun func) {
	og_key_callback = glfwSetKeyCallback(g_window, func);
	...
	if (og_key_callback) {
		glfwSetKeyCallback(g_window, key_callback_trampoline);
		return false;
	}
	return true;
}
