...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_ScrollToEdgeY         = 1 << 6,   // Force scrolling to _min/_max (used by Home/End) // FIXME-NAV: Aim to remove or reword, probably unnecessary
...
struct ImGuiContext
{
    ...
    float                   DragSpeedDefaultRatio;              // If speed == 0.0f, uses (_max-_min) * DragSpeedDefaultRatio
...
struct ImGuiTableColumn
{
    ...
    float                   WorkMinX;                       // Contents region _min ~(MinX + CellPaddingX + CellSpacingX1) == cursor start position when entering column
    float                   WorkMaxX;                       // Contents region _max ~(MaxX - CellPaddingX - CellSpacingX2)
