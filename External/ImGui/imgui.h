...
namespace ImGui
{
    ...
    IMGUI_API void          SetNextWindowSizeConstraints(const ImVec2& size_min, const ImVec2& size_max, ImGuiSizeCallback custom_callback = NULL, void* custom_callback_data = NULL); // set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1 for both _min and _max of same axis to preserve current size (which itself is a constraint). Use callback to apply non-trivial programmatic constraints.
    ...
    IMGUI_API ImVec2        GetWindowContentRegionMin();                                    // content boundaries _min for the full window (roughly (0,0)-Scroll), in window coordinates
    IMGUI_API ImVec2        GetWindowContentRegionMax();                                    // content boundaries _max for the full window (roughly (0,0)+Size-Scroll) where Size can be overridden with SetNextWindowContentSize(), in window coordinates
...
// - May be combined with SetNextWindowSizeConstraints() to set a _min/_max size for each axis (see "Demo->Child->Auto-resize with Constraints").
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_AlwaysClamp            = 1 << 4,       // Clamp value to _min/_max bounds when input manually with CTRL+Click. By default CTRL+Click allows going out of bounds.
...
// NB: For basic _min/_max size constraint on each axis you don't need to use the callback! The SetNextWindowSizeConstraints() parameters are enough.
...
struct ImGuiPayload
{
    ...
    char            DataType[32 + 1];   // Data type tag (short user-supplied string, 32 characters _max)
...
// This is optimized for efficient lookup (dichotomy into a contiguous buffer) and rare insertion (typically tied to user interactions aka _max once a frame)
